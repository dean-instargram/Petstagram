...
import postUploadModalSlice, {
  PostUploadModalState,
  addCurContentIndex,
  close,
  open,
} from '@/redux/postUploadModal';
...
export const PostUploadModal = ({
  ...
  const curContentIndex = useSelector(
    ({ postUploadModalSlice: state }: state) => {
      return state.curContentIndex;
    }
  );
  const nextBtnActived = useSelector(
    ({ postUploadModalSlice: state }: state) => {
      return state.nextBtnActived;
    }
  );
  const prevBtnActived = useSelector(
    ({ postUploadModalSlice: state }: state) => {
            ...
            >
              {hasHeader ? (
                <ModalHeader>
                  {nextBtnActived ? (
                    <button
                      className='modalPrevBtn'
                      onClick={movePrevContentHandler}
                    >
                      <svg
                        width='27'
                        height='23'
                  ...
                  <div className='modalTitle'>
                    {contentList[curContentIndex].modalTitle}
                  </div>
                  {nextBtnActived ? (
                    <button
                      className='modalNextBtn'
                      onClick={moveNextContentHandler}
                    >
                      다음
                    </button>
                  ) : null}
              ...
              <ModalContent>
                {contentList[curContentIndex].content}
              </ModalContent>
            </ModalView>
          </ModalBackdrop>
        ) : null}
